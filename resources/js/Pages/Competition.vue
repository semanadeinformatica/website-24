...
<template>
    <AppLayout title="Competição">
        <header class="flex flex-col items-center gap-4 p-4 text-center">
            <h2
                class="w-fit border border-solid border-black bg-2024-blue p-3 text-center text-2xl font-bold text-white shadow-md shadow-2024-black xl:text-3xl 2xl:text-4xl"
            >
                {{ competition.name }}
            </h2>
            <span
                class="inline-flex w-3/4 justify-center text-xl font-bold text-2024-black"
                >{{ competition.theme }}</span
            >
            <span
                class="inline-flex justify-center text-xl font-bold text-2024-black"
                >{{
                    formattedDate(
        ...
        <section
            class="relative mt-5 border-t border-black bg-2024-light-blue text-justify"
        >
            <h2
                class="absolute -top-9 left-1/2 -translate-x-1/2 transform border border-solid border-black bg-2024-blue p-3 text-2xl font-bold text-white shadow-md shadow-2024-bg xl:text-3xl 2xl:text-4xl"
            >
                Regulamento
            </h2>
            <div
        ...
        <template v-if="isOver">
            ...
            >
                <template v-if="competition.teams && competition.teams.length">
                    <p class="text-center text-2xl font-bold text-2024-black">
                        A competição já acabou, vê aqui quem participou
                    </p>
                    <span class="text-2xl font-bold text-2024-black">
                        Equipas: {{ competition.teams?.length }}
                    </span>
                    <div
                        class="flex w-4/5 flex-col border border-black shadow-lg shadow-2024-black md:w-1/2"
                    >
                        <div
                            v-for="team in competition.teams ?? []"
                            :key="team.id"
                            class="inline-flex w-full justify-between gap-1 p-4 text-lg even:bg-2024-blue even:bg-opacity-20"
                        >
                            <img
                                :src="
                ...
                <p v-else class="text-lg font-bold text-2024-black">
        ...
        <template v-else-if="!isAdmin($page.props.auth.user)">
            ...
            >
                ...
                <div
                    class="mx-10 flex h-full flex-1 flex-col items-center bg-2024-bg p-3 text-center text-3xl font-bold text-white sm:p-12"
                >
                    <PrimaryButton
                        color="orange"
                        shadow="teal"
            ...
            >
                ...
                <a
                    ...
                    class="relative mt-5 content-center justify-center border border-black bg-2024-black px-8 py-2 text-center text-2xl font-semibold text-white shadow-2024-blue transition-shadow hover:shadow-md active:shadow-none"
                >
                    Participar!
                </a>
            </section>
